<?php
    ...
    if (empty($errors)) {
        ...
        if ($stmtSuspension) {
            $type = 'suspension';
            $stmtSuspension->bind_param('is', $injuryId, $type);
            $stmtSuspension->execute();
            $result = $stmtSuspension->get_result();
            $suspension = $result ? $result->fetch_assoc() : null;
            $stmtSuspension->close();
        }
        if (!$suspension) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($errors)) {
        ...
        if (empty($errors)) {
            ...
            if ($stmtSave) {
                ...
                $errors[] = 'Unable to update suspension.';
            }
        }
    }

    if ($suspension && empty($errors)) {
        $stmtSuspension = $db->prepare(
            'SELECT id, lineup_match_id, team_side, player_name, reason, type
            FROM lineup_injuries
            WHERE id = ? AND type = ?
        ...
        if ($stmtSuspension) {
            ...
            $stmtSuspension->execute();
            $result = $stmtSuspension->get_result();
            $suspension = $result ? $result->fetch_assoc() : $suspension;
            $stmtSuspension->close();
        }
    }

    if ($selectedMatchId === 0 && $suspension) {
        $selectedMatchId = (int) $suspension['lineup_match_id'];
    }
                        ...
                        <div class="admin-form-grid">
                            <label class="admin-form-row">
                                ...
                                <select class="admin-select" name="lineup_match_id" required>
                                    <?php echo renderMatchOptions($lineups, (int) ($suspension['lineup_match_id'] ?? $selectedMatchId)); ?>
                                </select>
                            </label>
                            <label class="admin-form-row">
                                <span>Team Side</span>
                                <select class="admin-select" name="team_side">
                                    <option value="home" <?php echo ($suspension['team_side'] ?? 'home') === 'home' ? 'selected' : ''; ?>>Home</option>
                                    <option value="away" <?php echo ($suspension['team_side'] ?? 'home') === 'away' ? 'selected' : ''; ?>>Away</option>
                                </select>
                            </label>
                            <label class="admin-form-row">
                                <span>Player Name</span>
                                <input class="admin-input" type="text" name="player_name" value="<?php echo htmlspecialchars($suspension['player_name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" required>
                            </label>
                            <label class="admin-form-row">
                                <span>Reason</span>
                                <input class="admin-input" type="text" name="reason" value="<?php echo htmlspecialchars($suspension['reason'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                            </label>
                        </div>
                        <div class="admin-form-actions">
                            <a class="admin-link" href="../lineups.php?match_id=<?php echo (int) $selectedMatchId; ?>&section=suspensions">Back to lineups</a>
                            <button class="admin-btn admin-btn-primary" type="submit">Update Suspension</button>
                        </div>
                    </form>
                <?php endif; ?>
            </div>
